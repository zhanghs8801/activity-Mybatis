...
/*
 * 说明:原始作者为 '梅花雪疏影横斜'. 该跨平台版本by 蜡笔小新,欢迎交流,可在IE,firefox,Opera,Netscape版本上运行
 * csmfzu@163.com
 */
...
// " if(parent.WebCalendar.drag && drag){if(o.style.left=='')o.style.left=0;
// if(o.style.top=='')o.style.top=0;"+
// " o.style.left = parseInt(o.style.left) + window.event.clientX-cx;"+
// " o.style.top = parseInt(o.style.top) + window.event.clientY-cy;}}"+
// " document.onkeydown = function(){ switch(window.event.keyCode){ case 27 :
// parent.hiddenCalendar(); break;"+
// " case 37 : parent.prevM(); break; case 38 : parent.prevY(); break; case 39 :
// parent.nextM(); break; case 40 : parent.nextY(); break;"+
// " case 84 : document.forms[0].today.click(); break;} window.event.keyCode =
// 0; window.event.returnValue= false;}"+
// " dragStart = function(){cx=window.event.clientX; cy=window.event.clientY;
// drag=true;}</scr"+"ipt>"
// }

    strIframe += "<select name=tmpYearSelect  onblur='parent.hiddenSelect(this)' style='z-index:1;position:absolute;top:3px;left:18px;display:none'"+
    " onchange='parent.WebCalendar.thisYear =this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
    "<select name=tmpMonthSelect onblur='parent.hiddenSelect(this)' style='z-index:1; position:absolute;top:3px;left:74px;display:none'"+
   ...
   // if(WebCalendar.drag){strIframe += "onmousedown='dragStart()'
	// onmouseup='drag=false' onmouseout='drag=false'";}
    ...
    "    <tr><td><input style='width:20px;' name=prevYear title='向前翻 1 年&#13;快捷键：↑' onclick='parent.prevY()' type=button value='&lt;&lt;'"+
    "    onfocus='this.blur()'  ><input style='width:20px;' "+
    ...
    "    </td><td align=right><input style='width:20px;'  title='向后翻 1 月&#13;快捷键：→' name=nextMonth onclick='parent.nextM()' type=button value='&nbsp;&gt;'"+
    "    onfocus='this.blur()'><input style='width:20px;'  name=nextYear title='向后翻 1 年&#13;快捷键：↓' onclick='parent.nextY()' type=button value='&gt;&gt;'"+
    // style='meizz:expression(this.disabled=parent.WebCalendar.thisYear==9999)'
	// originally located in line 93,IE only
...
function calendar( e  ) // 主调函数
{
   ...
   // else WebCalendar.objExport = eval(arguments[0]);
    ...
    o.display = "";
    try{WebCalendar.iframe.document.body.focus();}catch(e){}
    var cw = WebCalendar.calendar.clientWidth, ch = WebCalendar.calendar.clientHeight;
    var dw = document.body.clientWidth, dl = document.body.scrollLeft, dt = document.body.scrollTop;
    if (document.body.clientHeight + dt - t - h >= ch){
    	o.top = ((p=="image")? t + h : t + h)+"px";
    }else{
    	o.top  = ((t - dt < ch) ? ((p=="image")? t + h : t + h) : t - ch)+"px";
    }
    if (dw + dl - l >= cw) {
    	o.left = (l)+"px";
    } else{
...
function writeCalendar() // 对日历显示的数据的处理程序
{
    ...
    // 修改过：下面这行代码不要写成WebCalendar.iframe.meizzYearHead.innerHTML = y +"
	// 年",有些浏览器会报错
    WebCalendar.iframe.document.getElementById('meizzYearHead').innerHTML=y +" 年";
    // 修改过：下面这行代码不要写成WebCalendar.iframe.meizzYearMonth.innerHTML = parseInt(m,
	// 10) +" 月",有些浏览器会报错
...
// 修改过：不要写成function document.onclick()这种格式
 document.onclick = function()
{
   if(WebCalendar.eventSrc != EventUtil.getEvent().target)
    hiddenCalendar();
}
// -->
